...
///
/// This might fail if the manager has not been initialized with a key
pub fn bls_public_key() -> impl Future<Item = Bn256PublicKey, Error = failure::Error> {
    let addr = SignatureManagerAdapter::from_registry();
...
///
/// This might fail if the manager has not been initialized with a key
pub fn bls_key_pair() -> impl Future<Item = (Bn256PublicKey, Bn256SecretKey), Error = failure::Error>
{
    let addr = SignatureManagerAdapter::from_registry();
