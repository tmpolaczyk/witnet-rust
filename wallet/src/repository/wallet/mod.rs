    ...
    T: Database,
{
    ...
    pub fn derive_and_persist_address(
        ...
        if persist_db {
            ...
            let info_db_key = keys::address_info(account, keychain, index);
            batch.put(&info_db_key, &info)?;
            batch.put(
                &keys::pkh(&pkh),
                &model::Path {
                    account,
    ...
    fn _persist_block_txns(
        ...
        // Persist addresses
        for address in addresses {
            let info_db_key = keys::address_info(account, address.keychain, address.index);
            batch.put(&info_db_key, &address.info)?;
            batch.put(
