...
pub fn websockets_actix_poc() {
    ...
    // Subscribe to new blocks, for testing
    JsonRpcClient::from_registry().do_send(JsonRpcSubscribeMsg::new(
        "newBlocks".to_string(),
        Value::Null,
    ));
    // Bug: ForwardUnsubscribe closes all the subscriptions
    JsonRpcClient::from_registry().do_send(JsonRpcSubscribeMsg::new(
        "newBlocks".to_string(),
        Value::Null,
    ));
    JsonRpcClient::from_registry().do_send(JsonRpcUnsubscribe {
        id: "21".to_string().into(),
    });
